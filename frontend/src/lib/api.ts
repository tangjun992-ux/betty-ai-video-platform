...
/** Absolutize a backend-relative media URL (/api/v1/media/...) to the API origin. */
...
// ─── Billing mode (testing phase: credit gate can be disabled server-side) ───
let _creditsEnforced: boolean | null = null;
/** True when the backend enforces credit deduction. False in the testing phase. */
export async function isCreditsEnforced(): Promise<boolean> {
  if (_creditsEnforced !== null) return _creditsEnforced;
  try {
    const r = await fetch(`${API_BASE}/system/capabilities`);
    if (r.ok) {
      const d = await r.json();
      _creditsEnforced = d.credits_enforced !== false; // default true when field missing
      return _creditsEnforced;
    }
  } catch { /* fall through */ }
  _creditsEnforced = true;
  return _creditsEnforced;
}
...
export async function submitGeneration(req: GenerateRequest): Promise<GenerateResponse> {
  ...
  if (!res.ok) {
    ...
    if (typeof d === "string") {
      ...
      message = (d as any).message || message;
      code = (d as any).code;
    }
