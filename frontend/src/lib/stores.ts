...
export const useCreationStore = create<CreationState>()(
  persist(
    ...
    {
      name: "creation-store-v2",
      // referenceFiles contains runtime File objects and blob URLs which cannot be
      // safely serialized to localStorage. Skip them on persist/rehydrate.
      partialize: (state) =>
        Object.fromEntries(
          Object.entries(state).filter(([key]) => key !== "referenceFiles")
        ) as any,
    }
  )
);
