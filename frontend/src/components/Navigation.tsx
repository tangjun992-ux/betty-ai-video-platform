...
// 工具下拉(顶部导航的 Tools 菜单)
const toolMenu = [
  { label: "图片编辑", href: "/create/image?tool=editor" },
    ...
    >
      <div className="mx-auto max-w-[1440px] h-full px-4 md:px-6 flex items-center gap-4">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <Link
            href="/pricing"
            className="hidden sm:inline-flex items-center gap-1.5 h-9 px-4 rounded-lg bg-brand text-brand-foreground text-sm font-semibold shadow-button-glow hover:bg-brand-strong hover:-translate-y-px transition-all"
          >
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-48">
                ...
                <DropdownMenuItem onClick={() => router.push("/dashboard")}>
                  <LayoutDashboard className="w-4 h-4 mr-2" />仪表盘                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => router.push("/settings")}>
                  <Settings className="w-4 h-4 mr-2" />设置
                </DropdownMenuItem>
                <DropdownMenuSeparator />
                <DropdownMenuItem onClick={handleLogout} className="text-destructive">
                  <LogOut className="w-4 h-4 mr-2" />退出登录                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
          ) : (
            <Link
