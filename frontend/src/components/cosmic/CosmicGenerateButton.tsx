...
const variantStyles: Record<ButtonVariant, string> = {
  primary: cn(
    // Solid brand background (Studio v5: zero rainbow gradients)
    "bg-brand",
    "text-brand-foreground",
    // Glow
    "shadow-button-glow",
    // Hover
    "hover:bg-brand-strong hover:shadow-button-glow-lg",
    // Active
        ...
        {variant === "primary" && !isDisabled && (
          <div className="absolute inset-0 rounded-xl opacity-0 hover:opacity-100 transition-opacity duration-500 pointer-events-none">
            <div
              ...
              style={{
                background:
                  "radial-gradient(circle at 50% 0%, hsl(var(--brand-strong) / 0.18), transparent 70%)",
              }}
            />
          </div>
        )}
