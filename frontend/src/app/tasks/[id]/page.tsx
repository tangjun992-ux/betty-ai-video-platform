...
import { API_BASE, getTaskStatus, listTasks, resolveMediaUrl } from "@/lib/api";
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {results.map((r: any, idx: number) => (
              <div key={idx} className="bg-dark-900/50 border border-dark-800 rounded-xl overflow-hidden">
                {r.url ? (
                  r.type === "video" ? (
                    <video src={resolveMediaUrl(r.url)} controls className="w-full" />
                  ) : (
                    <img src={resolveMediaUrl(r.url)} alt="result" className="w-full h-64 object-cover" />
                  )
                ) : (
                ...
                <div className="p-3 flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    {r.url && (
                      <button onClick={() => { const a = document.createElement("a"); a.href = resolveMediaUrl(r.url); a.download = ""; a.target = "_blank"; a.click(); }}
