    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
        ...
        <p className="text-sm text-text-secondary mb-4">
          原生 Kling Motion Control + 可选 Lipsync 口播分轨。对标「表演驱动」工作流，但不是 Runway Act-One 编码器。        </p>
      ...
      <button
        ...
        onClick={submit}
        className={cn(
          "w-full py-3.5 rounded-2xl bg-brand text-brand-foreground font-semibold disabled:opacity-40 flex items-center justify-center gap-2",
        )}
      >
        {submitting ? <Loader2 className="w-5 h-5 animate-spin" /> : <Clapperboard className="w-5 h-5" />}
        开启 Performance Drive
      </button>
    </div>
  );
}
