...
function stripLeadingEmoji(label: string) {
  return label
    .replace(
      /^[\s]*(?:[\u{1F300}-\u{1F9FF}]|[\u{2600}-\u{26FF}]|[\u{2700}-\u{27BF}]|[\u{1F000}-\u{1F02F}]|[\u{1F0A0}-\u{1F0FF}]|[\u{1F100}-\u{1F64F}]|[\u{1F680}-\u{1F6FF}]|[\u{1F1E0}-\u{1F1FF}]|[\u{FE00}-\u{FE0F}])+[\s]*/u,
      ""
    )
    .trim();
}
          ...
          {[
            ...
            <button
              ...
              className={cn(
                ...
                mediaFilter === f.key
                  ? "bg-brand text-brand-foreground shadow-button-glow"
        ...
        {styleOptions.length > 0 && (
          <div className="flex flex-wrap justify-center gap-2">
            {[{ key: "all", label: "全部风格" } as StyleOption, ...styleOptions.filter((s) => s.key !== "all").slice(0, 15)].map((s) => (
              ...
              >
                {stripLeadingEmoji(s.label) || s.label}
              </button>
            ))}
          </div>
        )}
                ...
                >
                  ...
                  <div className="absolute top-2.5 left-2.5 flex gap-1.5 flex-wrap max-w-[70%]">
                    ...
                    {(item.is_seed || item.is_demo) && (
                      <span className="px-2 py-0.5 bg-brand/75 backdrop-blur-md rounded-full text-[10px] text-brand-foreground font-semibold" title="平台示例内容，非真实用户创作">
                        示例
                      </span>
                    )}
                  ...
                  <div className="absolute top-2.5 right-2.5">
                    <span className="px-2 py-0.5 bg-black/55 backdrop-blur-md rounded-full text-[10px] text-brand font-medium">
                      ⚡{item.credits_cost}
                    </span>
                  </div>
            ...
            >
              ...
              <div className="lg:w-80 flex-shrink-0 flex flex-col p-5 gap-4 overflow-y-auto">
                ...
                <div className="flex flex-wrap gap-1.5">
                  ...
                  {lightbox.media_type === "video" && <span className="badge-cyan">视频</span>}
                  {(lightbox.is_seed || lightbox.is_demo) && (
                    <span className="badge" title="平台示例内容">示例</span>
                  )}
                  {lightbox.resolution && <span className="badge">{lightbox.resolution}</span>}
                  <span className="badge-cyan">⚡{lightbox.credits_cost}</span>
                </div>
