...
import {
  Plus, MessageSquare, ImagePlus, Sparkles, Wand2, Lightbulb,
  ...
  Download, Trash2, StopCircle, CornerDownLeft, Star, ArrowUp, ChevronDown,
} from "lucide-react";
import { cn } from "@/lib/utils";
import { API_BASE, isCreditsEnforced } from "@/lib/api";
...
export default function AgentPage() {
  ...
  const runVariantGallery = async (cards?: typeof variantCards, dryRun = false) => {
    ...
    try {
      if (!dryRun) {
        ...
        // 测试阶段（后端 CREDITS_ENFORCED=false）跳过付费拦截
        if (await isCreditsEnforced()) {
          try {
  ...
  const ensureCreditsForReal = async (needed: number): Promise<boolean> => {
    // 测试阶段（后端 CREDITS_ENFORCED=false）跳过付费拦截
    if (!(await isCreditsEnforced())) return true;
    try {
      const r = await fetch(`${API_BASE}/models/pricing/user`);
      if (!r.ok) return true;
      const d = await r.json();
  ...
  // ── 测试入口：领取测试积分（仅测试/开发环境） ──
  const claimTestCredits = async () => {
    try {
      const r = await fetch(`${API_BASE}/billing/dev/grant?amount=100`, { method: "POST" });
      const d = await r.json().catch(() => ({}));
      if (r.ok && d.ok) {
        setUserCredits(d.credits);
        toast.success("已领取测试积分", `+${d.granted} 积分，当前余额 ${d.credits}`);
      } else {
        toast.error("领取失败", d.detail || "测试领取不可用");
      }
    } catch (e: any) {
      toast.error("领取失败", e?.message || "网络错误");
    }
  };
        ...
        <div className="max-w-3xl w-full mx-auto px-4 py-8 flex-1">
          ...
          <div className="text-center mb-6">
            <div className="flex items-center justify-center gap-2 mb-3 flex-wrap">
              ...
              <button
                type="button"
                onClick={claimTestCredits}
                title="测试阶段免费领取积分（仅测试/开发环境）"
                className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium border border-cosmic-border text-text-secondary hover:text-brand-strong hover:border-brand/40 transition-colors"
              >
                <Coins className="w-3.5 h-3.5" /> 领取测试积分
              </button>
            </div>
          ...
          {/* Input — dzine/Yapper 式一体化画布输入（与首页同款） */}
          <div className="rounded-2xl border border-cosmic-border bg-cosmic-surface shadow-elevation-lg overflow-visible mb-3">
            {/* 文本区（左上上传图标） */}
            <div className="relative">
              ...
              <textarea
                ...
                rows={3}
                className="w-full bg-transparent border-none pl-12 pr-4 pt-3 pb-2 text-[15px] leading-relaxed placeholder:text-text-tertiary focus:outline-none resize-none"
              />
            </div>
            ...
            {refImageUrl && (
              <div className="flex items-center gap-2 px-3 pb-2.5">
                <div className="flex items-center gap-1.5 px-1.5 py-1 rounded-lg bg-brand/10 text-brand-strong text-xs">
                  <img src={resolveMedia(refImageUrl)} alt="ref" className="w-5 h-5 rounded object-cover" />
                  参考图
                  <button onClick={() => { setRefImageUrl(null); setRefImage(false); }} className="hover:text-destructive"><X className="w-3 h-3" /></button>
                </div>
              </div>
            )}

            {/* 底部工具条：导演大脑 + 时长 + 成片模式 + 字数/积分 + 圆形提交（一体化，无内框） */}
            <div className="flex items-center gap-2 flex-wrap px-3 py-2.5 bg-cosmic-subtle/35 rounded-b-2xl">
              {/* 导演大脑 — 无独立边框，直接在工具条上 */}
              {([
                { id: "fast", label: "快速" },
                { id: "quality", label: "深度" },
                ...
                <button key={b.id} type="button" onClick={() => setBrain(b.id)} title="导演大脑"
                  className={cn("px-2.5 py-1 rounded-lg text-[11px] font-medium transition-colors",
                    brain === b.id ? "bg-cosmic-elevated text-text-primary" : "text-text-tertiary hover:text-text-secondary hover:bg-cosmic-elevated/50")}>
                  {b.label}
                </button>
              ))}
              <div className="flex items-center gap-1 text-xs text-text-secondary">
                ...
                <select value={duration} onChange={(e) => setDuration(+e.target.value)}
                  ...
                  {[5, 10, 15, 30].map((d) => <option key={d} value={d} className="bg-cosmic-elevated">{d}s</option>)}
                </select>
              </div>
              <button
                type="button"
                ...
                className={cn(
                  ...
                  minimal
                    ? "bg-brand/10 text-brand-strong border-brand/30"
              ...
              <div className="ml-auto flex items-center gap-2.5">
                ...
                <span className="hidden sm:inline-flex items-center gap-1 text-[11px] text-text-tertiary select-none">
                  <span className="w-1.5 h-1.5 rounded-full bg-brand" />{userCredits ?? "…"}
                </span>
                <button
                  type="button"
                  onClick={() => makePlan()}
                  disabled={!brief.trim() || phase === "planning"}
                  aria-label="开始导演"
                  title="开始导演（⌘/Ctrl + ⏎）"
                  data-testid="agent-plan-btn"
                  className="w-9 h-9 rounded-full bg-brand hover:bg-brand-strong text-brand-foreground flex items-center justify-center transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  {phase === "planning" ? (
          ...
          {/* ── 快捷动作 + 高级选项 ── */}
          <div className="flex flex-wrap items-center gap-2 mb-4" data-testid="agent-controls">
            <button onClick={polishBrief} disabled={!brief.trim() || !!composerBusy}
              className="chip h-8 px-3 text-xs disabled:opacity-40">
            ...
            <button
              ...
              className={cn("chip h-8 px-3 text-xs", showAdvanced && "text-brand-strong border-brand/30 bg-brand/5")}
            >
              高级选项
              <ChevronDown className={cn("w-3.5 h-3.5 opacity-60 transition-transform", showAdvanced && "rotate-180")} />
            </button>

            {/* 成片模板（下拉，联动 brief/时长/场景/投放位） */}
            {briefTemplates.length > 0 && phase === "idle" && (
              <div className="relative" data-testid="agent-brief-templates">
                <button
                  type="button"
                  onClick={() => setTplOpen((v) => !v)}
                  className={cn("chip h-8 px-3 text-xs", tplOpen && "text-brand-strong border-brand/30 bg-brand/5")}
                >
                  成片模板
                  <ChevronDown className={cn("w-3.5 h-3.5 opacity-60 transition-transform", tplOpen && "rotate-180")} />
                </button>
                {tplOpen && (
                  <>
                    <div className="fixed inset-0 z-40" onClick={() => setTplOpen(false)} />
                    <div className="absolute left-0 top-full mt-1.5 z-50 w-72 rounded-xl border border-cosmic-border bg-cosmic-elevated shadow-elevation-lg p-1.5 max-h-80 overflow-y-auto">
                      {briefTemplates.map((tpl) => (
                        <button
                          key={tpl.id}
                          type="button"
                          onClick={() => applyTemplate(tpl)}
                          className="w-full flex flex-col items-start gap-0.5 px-3 py-2 rounded-lg text-left transition-colors hover:bg-cosmic-subtle"
                        >
                          <span className="text-[13px] font-medium text-text-primary">{tpl.label}</span>
                          {tpl.cta ? <span className="text-[11px] text-text-tertiary">{tpl.cta}</span> : null}
                        </button>
                      ))}
                    </div>
                  </>
          ...
          <AnimatePresence>
            {showAdvanced && (
              ...
              >
                <div className="rounded-2xl border border-cosmic-border bg-cosmic-surface p-4 mb-4 space-y-3.5">
                  {/* 身份锁 */}
                  <div className="flex items-center gap-2 flex-wrap" data-testid="agent-identity-lock">
                    <span className="text-[11px] text-text-tertiary shrink-0 w-14">身份锁</span>
                    <div className="inline-flex items-center gap-0.5 p-0.5 rounded-lg bg-cosmic-deep border border-cosmic-border">
                      {([
                        ...
                        <button key={opt.id} type="button" onClick={() => setIdentityLock(opt.id)}
                          ...
                          className={cn("px-2.5 py-1 rounded-md text-[11px] font-medium transition-colors",
                            identityLock === opt.id ? "bg-cosmic-subtle text-text-primary" : "text-text-tertiary hover:text-text-secondary")}>
                          {opt.label}
                        </button>
                      ))}
                    </div>
                  ...
                  {exportSpecs.length > 0 && (
                    <div className="flex items-center gap-2 flex-wrap" data-testid="agent-export-placement">
                      <span className="text-[11px] text-text-tertiary shrink-0 w-14">投放位</span>
                      <div className="inline-flex flex-wrap items-center gap-0.5 p-0.5 rounded-lg bg-cosmic-deep border border-cosmic-border">
                        {exportSpecs.map((sp) => (
                          <button key={sp.id} type="button"
                            ...
                            className={cn("px-2.5 py-1 rounded-md text-[11px] font-medium transition-colors whitespace-nowrap",
                              exportPlacement === sp.id ? "bg-cosmic-subtle text-text-primary" : "text-text-tertiary hover:text-text-secondary")}>
                            {sp.label}
                          </button>
                        ))}
                      </div>
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
                ...
                <div className="flex gap-2">
                  ...
                  <button type="button" disabled={variantRunning}
                    ...
                    className="px-2.5 py-1 rounded-lg text-[11px] bg-brand text-brand-foreground hover:bg-brand-strong">
                    真实并行出片 · 画廊选优
                  </button>
                </div>
              </div>
                      ...
                      <div className="p-3 space-y-2">
                        ...
                        <button type="button" disabled={!v.final && !v.plan}
                          ...
                          className={cn(
                            ...
                            selected ? "bg-brand text-brand-foreground" : "border border-brand/40 text-brand hover:bg-brand/10",
                          )}>
                          {selected ? "已采用" : "采用此变体"}
                        </button>
                      </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {SCENARIOS.map((sc) => (
                  <button key={sc.title} onClick={() => startScenario(sc)}
                    className="group relative flex items-start gap-3 p-4 rounded-xl bg-cosmic-surface border border-cosmic-border hover:border-cosmic-border-hover hover:bg-cosmic-elevated transition-colors text-left">
                    <span className="icon-tile w-10 h-10 flex-shrink-0">
                      <sc.icon className="w-[18px] h-[18px]" />
                    </span>
                    <div className="flex-1 min-w-0">
                    ...
                    <span className="absolute right-3 top-1/2 -translate-y-1/2 inline-flex items-center gap-1 text-[11px] font-medium text-brand-strong opacity-0 group-hover:opacity-100 transition-opacity">
              ...
              <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="mt-5" data-testid="agent-plan">
                ...
                {phase !== "running" && plan.steps.some((s) => s.action === "video") && (
                  <button onClick={() => refine("加一个镜头")} disabled={refining}
                    ...
                    <Plus className="w-3.5 h-3.5" /> 加一个分镜                  </button>
                )}
                  ...
                  <div className="mt-4">
                    ...
                    <div className="flex items-center gap-2 rounded-xl border border-cosmic-border/50 bg-cosmic-surface/40 p-1.5 focus-within:border-brand/40 transition-colors">
                      ...
                      <input value={refineText} onChange={(e) => setRefineText(e.target.value)}
                        ...
                        placeholder="告诉导演如何调整：把第 2 镜改掉 / 加个镜头 / 换成 Seedance Fast / 竖屏…"
                      ...
                      <button onClick={() => refine()} disabled={!refineText.trim() || refining}
                        className={cn("flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium transition-all flex-shrink-0",
                          refineText.trim() && !refining ? "bg-brand text-brand-foreground hover:bg-brand-strong" : "bg-cosmic-surface text-text-secondary")}>
                        ...
                        {realAvailable !== false ? (
                          <>
                            <button
                              ...
                              className="flex items-center justify-center gap-2 flex-1 py-3 rounded-xl text-sm font-semibold bg-brand text-brand-foreground hover:bg-brand-strong shadow-button-glow transition-all"
                            >
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-6">
                ...
                <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                  ...
                  <div className="flex items-center justify-between px-4 py-2.5 border-b border-brand/15">
                    ...
                    <a href={fMedia} download target="_blank" rel="noreferrer"
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-brand text-brand-foreground text-xs font-medium hover:bg-brand-strong transition-colors">
                    ...
                    <div className="px-4 py-3 border-t border-brand/15 bg-gradient-to-r from-brand/[0.06] to-accent-violet/[0.04] flex flex-col sm:flex-row sm:items-center gap-3">
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-text-secondary mt-0.5">
                          当前余额 {userCredits ?? "…"} 积分。升级套餐可解锁更多真实生成与商业授权。                        </p>
                      </div>
                      <div className="flex gap-2">
                        <button
                          onClick={() => setPayTarget({ kind: "plan", id: "personal", cycle: "monthly" })}
                          className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-brand text-brand-foreground text-xs font-semibold hover:bg-brand-strong transition-colors"
                        >
              ...
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="text-sm font-semibold">
                  {running ? "逐镜产出中" : "分镜素材"} {shotAssets.length} 个                  {dryRunMode && <span className="text-[11px] text-amber-600 ml-1">(本地预览 · 非真实模型)</span>}
                  {totalMs != null && !running && <span className="text-[11px] text-text-tertiary ml-2">共用时 {(totalMs / 1000).toFixed(1)}s</span>}
                </h3>
              </div>
              {dryRunMode && (
                <div className="mb-3 rounded-xl border border-amber-400/40 bg-amber-500/10 px-3 py-2 text-xs text-amber-800 dark:text-amber-200">
                  免费预览不会调用计划里的 GPT Image / ElevenLabs / Kling。数字人步骤使用本地占位人像 + Ken Burns 动效。                  要点「真实生成」才会按规划扣积分跑真实口型。                </div>
              )}
                  ...
                  return (
                    <motion.div key={a.step_id || i} initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }}
                      ...
                      <div className="relative aspect-video bg-gradient-to-br from-cosmic-surface to-cosmic-border/40 flex items-center justify-center overflow-hidden">
                        ...
                        {a.step_id && !running && (
                          <button onClick={() => regenerate(a)} disabled={busy} title="重生成此镜头"
