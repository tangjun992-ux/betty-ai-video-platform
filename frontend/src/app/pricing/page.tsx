...
const PLANS = [
  {
    ...
    color: "from-cosmic-border-hover to-text-tertiary",
...
const FAQS = [
  ...
  { q: "如何获得更多 Credits？", a: "Credits 按订阅档位每月发放。升级更高档位可立即获得新旧档位差额的 Credits（按比例计费）。也可随时在定价页购买一次性的 Credits 充值包。" },
    ...
    <div className="max-w-6xl mx-auto px-4 py-12">
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
        ...
        <div role="radiogroup" aria-label="计费周期" className="inline-flex items-center gap-3 p-1 rounded-full bg-cosmic-subtle border border-cosmic-border">
          <button role="radio" aria-checked={!yearly} onClick={() => setYearly(false)}
            className={`px-5 py-2 rounded-full text-sm font-semibold transition-all ${!yearly ? "bg-brand text-brand-foreground shadow-button-glow" : "text-text-secondary hover:text-brand"}`}>月付</button>
          <button role="radio" aria-checked={yearly} onClick={() => setYearly(true)}
            className={`px-5 py-2 rounded-full text-sm font-semibold transition-all flex items-center gap-1.5 ${yearly ? "bg-brand text-brand-foreground shadow-button-glow" : "text-text-secondary hover:text-brand"}`}>
            年付<span className="badge-cyan text-[10px]">省 20%</span>
          </button>
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-16">
        {PLANS.map((plan, i) => (
          <motion.div key={plan.name} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }}
            ...
            {plan.popular && (
              <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full bg-brand text-brand-foreground text-xs font-semibold shadow-button-glow">Most Popular</div>
            )}
            ...
            <button onClick={() => subscribe(plan.id)} disabled={!!busy}
              ...
              plan.popular ? "bg-brand text-brand-foreground hover:bg-brand-strong shadow-button-glow" : "bg-cosmic-subtle border border-cosmic-border text-text-primary hover:bg-cosmic-border"}`}>
        ...
        {/* Max 档 credits slider */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.24 }}
          className="relative rounded-2xl border border-brand/20 bg-brand/[0.03] p-6 flex flex-col">
          <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full bg-brand text-brand-foreground text-xs font-semibold shadow-button-glow">Best Value</div>
          <div className="flex items-center gap-2.5 mb-4">
            <div className="w-2 h-2 rounded-full bg-gradient-to-br from-brand to-brand-strong" />
          ...
          {yearly && <p className="text-xs text-brand mb-2">按年计费 ${(maxTier.yearly * 12).toFixed(0)}/年</p>}
          ...
          <input type="range" min={0} max={MAX_TIERS.length - 1} step={1} value={maxIdx}
            ...
            className="w-full accent-brand mb-1 cursor-pointer" />
          ...
          <button onClick={() => subscribe("max")} disabled={!!busy}
            className="w-full py-2.5 rounded-xl text-sm font-semibold text-center bg-brand/[0.08] border border-brand/30 text-brand hover:bg-brand/[0.14] transition-all active:scale-[0.98] inline-flex items-center justify-center gap-1.5 disabled:opacity-60">
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="max-w-3xl mx-auto mb-16">
              ...
              {CREDIT_USAGE.map((item, i) => (
                <tr key={i} className="border-b border-cosmic-border last:border-0 hover:bg-cosmic-subtle transition-colors">
                  ...
                  <td className="px-5 py-3">
                    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs ${item.type === "图片" ? "bg-brand/10 text-brand" : "bg-brand/10 text-brand"}`}>{item.type}</span>
                  </td>
        ...
        <p className="text-xs text-text-secondary text-center mt-3 flex items-center justify-center gap-1">
          <Info className="w-3 h-3" />生成前始终显示精确 Credits 成本；实际消耗随模型与时长浮动        </p>
      </motion.div>
