...
def generate_video_task(
    ...
    try:
        ...
        error = rd.get("error")
        if error and _is_moderation_error(error):
            # Content-safety rejection → retry once with a sanitized prompt (same model).
            try:
                _update_task(db_task_id, current_stage="sanitize_retry")
                _broadcast_progress(db_task_id, 55, "sanitize_retry", "内容安全过滤，正在用更稳妥的表达重试...")
                retry_prompt = _sanitize_prompt(prompt)
                result2 = _run_async(
                    adapter.generate_video(
                        prompt=retry_prompt, model_id=model,
                        image_url=image_url or (ref_images[0] if ref_images else None),
                ...
                rd2 = result2.to_dict() if hasattr(result2, "to_dict") else result2
                if not rd2.get("error"):
                    rd = rd2
                    error = None
                    logger.info("[video] sanitize-retry succeeded for task %s", db_task_id)
            except Exception as se:  # noqa: BLE001
                logger.warning("[video] sanitize-retry failed for task %s: %s", db_task_id, se)
        if error:
...
def _is_moderation_error(err: str) -> bool:
    """Detect provider-side content-safety/moderation rejections (incl. KIE 'sensitive')."""
    e = (err or "").lower()
    return any(w in e for w in (
        "guardrails", "nudity", "sexuality", "erotic", "inappropriate", "content policy",
        "safety system", "violates", "sensitive", "nsfw", "moderation", "policy violation",
    ))


_SOFT_BLOCKLIST = [
    ("落魄", "低调"), ("抑郁", "沉静"), ("痛苦", "感慨"), ("绝望", "沉思"), ("悲伤", "温柔"),
    ("哭泣", "含泪"), ("崩溃", "动容"), ("黑暗", "夜色"), ("死亡", "告别"), ("自杀", "告别"),
    ("出轨", "误会"), ("暴力", "冲突"), ("血腥", "紧张"), ("杀害", "告别"), ("酗酒", "疲惫"),
]


def _sanitize_prompt(prompt: str) -> str:
    """Soften a prompt that tripped the provider's content filter, keeping the scene's
    intent but moving it into clearly-safe territory (wholesome/uplifting tone)."""
    p = prompt or ""
    for a, b in _SOFT_BLOCKLIST:
        p = p.replace(a, b)
    return (
        "safe for work, wholesome, family friendly, uplifting hopeful emotional tone, "
        "no dark/disturbing/explicit content. " + p
    )
...
def _translate_error(error_msg: str) -> str:
    ...
    if any(w in err_lower for w in ("guardrails", "nudity", "sexuality", "erotic", "inappropriate", "content policy", "safety system", "sensitive information", "nsfw")):
        return "内容不合规：您的提示词包含不当内容，已被 AI 安全系统拦截。请修改后重试。"
    if "violates" in err_lower or "sensitive" in err_lower:
