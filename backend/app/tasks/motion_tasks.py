...
from app.tasks.common import broadcast_progress, chdir_backend_root, run_async, update_task
...
def process_motion_task(self, db_task_id: str, model: str, prompt: str, params: dict):
    ...
    # Stage 1: Loading
    broadcast_progress(db_task_id, 5, "loading", "加载运动迁移模型...")
    update_task(db_task_id, status="generating", progress=5, current_stage="loading")

    # Stage 2: Analyzing reference motion
    broadcast_progress(db_task_id, 15, "analyzing", "分析参考视频动作轨迹...")
    update_task(db_task_id, progress=15, current_stage="analyzing")
    time.sleep(1.5)

    # Stage 3: Transfer motion
    broadcast_progress(db_task_id, 35, "transferring", "将动作迁移到目标人物...")
    update_task(db_task_id, progress=35, current_stage="transferring")
    ...
    try:
        ...
        if adapter:
            broadcast_progress(db_task_id, 50, "generating", "Seedance 视频生成中...")
            update_task(db_task_id, progress=50, current_stage="generating")
            ...
            broadcast_progress(db_task_id, 100, "completed", "运动控制生成完成！")
...
def _simulate_completion(db_task_id: str, image_url: str, video_url: str, prompt: str, style: str):
    ...
    broadcast_progress(db_task_id, 100, "completed", "运动控制完成（模拟模式）")
