...
async def deduct_credits(
    ...
    *,
    ...
    # Testing-phase kill switch: when the billing system is not finalized we can
    # disable enforcement so generation is never blocked by the credit gate.
    from app.config import settings
    if not getattr(settings, "CREDITS_ENFORCED", True):
        logger.info(
            "credits: enforcement disabled (testing) — allow task=%s cost=%s user=%s",
            task_id, cost, user_id,
        )
        return True
