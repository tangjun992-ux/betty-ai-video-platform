...
class PromptRouter:
    ...
    def analyze(self, prompt: str, media_type_hint: str = "auto",
        ...
        # Media type detection — explicit user hint (from the creation page the
        # user is on) always wins; keyword heuristics are only used for "auto".
        ...
        elif video_count > 0 and video_count >= image_count:
            analysis.media_type = MediaType.VIDEO
        elif image_count > 0 and image_count > video_count:
            analysis.media_type = MediaType.IMAGE
        else:
            analysis.media_type = MediaType.AUTO

        # Quality detection — an explicit tier chosen in the UI always wins;
        # keyword heuristics are only a fallback for non-specific hints.
        ...
        elif quality_hint == "fast":
            analysis.quality = QualityTier.FAST
        elif quality_hint == "balanced":
            analysis.quality = QualityTier.BALANCED
        elif hq_count > fq_count:
            analysis.quality = QualityTier.HIGH
        elif fq_count > hq_count:
            analysis.quality = QualityTier.FAST
        else:
            analysis.quality = QualityTier.BALANCED
