...
def _plan_shots(brief: str, n: int, scenario: str | None, styles: list[str], brain: str | None = None) -> list[dict]:
    """Return a differentiated per-shot plan `[{title, prompt, camera}]`.

    Tries the director LLM for a true dynamic storyboard (Yapper/dzine parity);
    on any failure falls back to a content-aware deterministic breakdown so every
    shot's visual focus actually differs. Never the same full brief for every shot.
    """
    from app.services.director_brain import (
        _deterministic_shot_list, _llm_credentials, resolve_brain_model,
    )
    b = (brief or "").strip()
    n = max(1, int(n or 1))
    if not b:
        ...
        try:
            ...
            with _httpx.Client(timeout=25) as client:
                resp = client.post(
                    ...
                    json={
                        "model": resolve_brain_model(brain) or "gpt-4o-mini",
                        "messages": [
            ...
            logger.info("[director] LLM storyboard unavailable, using content-aware fallback: %s", e)
    return _deterministic_shot_list(b, n, styles)
...
def _shot_clip(model: dict | None) -> int:
    """Per-shot clip length (seconds) from the model's capability.

    Top-tier directors split long videos into model-optimal clips (not a fixed 5s).
    Clamped to [5, 15] so cross-shot identity stays manageable and cost/latency sane.
    """
    caps = (model or {}).get("capabilities", {}) or {}
    max_clip = int(caps.get("max_duration_s") or 0)
    if max_clip <= 0:
        max_clip = 15
    return max(5, min(15, max_clip))


def _n_shots(duration: int, model: dict | None = None) -> int:
    """Shots needed to cover `duration` seconds using the model's clip length."""
    clip = _shot_clip(model)
    n = -(-int(duration) // clip)  # ceil(duration / clip)
...
class DirectorPlanner:
    def plan(self, brief: str, has_ref_image: bool = False, duration: int = 5,
        ...
        # Placement policy (aspect / packaging defaults). Duration priority:
        #   1) explicit UI duration param, 2) duration written in the brief, 3) placement default.
        # Placement duration_max is only a *default*, NOT a hard cap on an explicit user request.
        ...
        if _has(brief, ["快速成片", "最短路径", "minimal", "quick direct", "只要视频", "不要配音"]):
            minimal = True
        # Parse an explicit duration from the brief ("30秒 / 30s / 30 seconds").
        dm = re.search(r"(\d{1,3})\s*(?:秒|s\b|sec|seconds?)", brief, re.I)
        explicit_dur = int(dm.group(1)) if dm else int(duration or 0)
        if explicit_dur:
            # Respect the user's stated length (sane absolute cap), not the placement max.
            duration = max(5, min(explicit_dur, 120))
        else:
            duration = clamp_duration(int(placement.get("duration_default") or 15), placement)
        ...
        def _add_video_shots(
            ...
            *, hero_id: str | None = None,
            ...
            clip = _shot_clip(vid)  # model-aware per-shot duration
            # Differentiated shot plan (LLM storyboard if available, else content-aware).
            shot_list = _plan_shots(brief, n, scenario, styles, brain=None)
            for i in range(n):
                beat_cn, beat_desc, cam_en = beats[i % len(beats)]
                shot = shot_list[i] if i < len(shot_list) else None
                shot_title = (shot.get("title") if shot else None) or beat_cn
                shot_body = (shot.get("prompt") if shot else None) or _shot_prompt(
                ...
                sv = DirectorStep(
                    id=sid(), action="video",
                    title=f"分镜 {i+1}/{n} · {shot_title}",
                    model_id=vid["id"], model_name=vid["display_name"],
                    reason=f"{beat_desc}，选 {vid['display_name']}",
                    prompt=shot_body + id_prompt,
                    depends_on=deps, est_credits=_credits_of(vid, "video", clip),
                    params={
                        "aspect_ratio": aspect, "duration": clip, "shot": i + 1,
        ...
        if scenario == "talking_avatar" or (not scenario and _has(brief, _TALK_KW)):
            ...
            s_voice = DirectorStep(id=sid(), action="audio", title="AI 配音 · 旁白 (TTS)",
                ...
                params={"script": brief, "voice_id": voice_id, "talking": True, "duration": duration})
            ...
            if scenario == "ugc":
                ...
                n = min(3, max(2, _n_shots(duration, vid)))
                vid_aspect = "9:16"
                img_aspect = "9:16"
                ...
                n = max(4, _n_shots(duration, vid))  # commercial never collapses to 1 shot
                vid_aspect = "16:9"
                ...
                n = min(3, max(2, _n_shots(duration, vid))) if duration <= 15 else _n_shots(duration, vid)
                vid_aspect = "16:9"
                img_aspect = "16:9"
            ...
            for mid in ("seedance-2.0",):
                ...
                if hit:
                    vid = hit
                    break
            # Top-tier short drama needs multi-beat even in「快速成片」(镜数跟随时长, 下限2)
            n = 2 if minimal else max(2, min(_n_shots(duration, vid), 4))
            ...
            n = 2 if minimal else max(3, min(_n_shots(duration, vid), 4))
            ...
            if has_ref_image:
                ...
                vid = _pick_model("video", styles, "balanced")
                n = 1 if minimal else _n_shots(duration, vid)
                clip = _shot_clip(vid)
                prev = enh_id
                for i in range(n):
                    ...
                    sv = DirectorStep(id=sid(), action="video",
                        ...
                        depends_on=[prev], est_credits=_credits_of(vid, "video", clip),
                        params={"aspect_ratio": vid_aspect, "duration": clip, "shot": i + 1})
                    steps.append(sv)
                    prev = sv.id
            else:
                intent = "video_from_text"
    ...
    async def _run_step(self, step: DirectorStep) -> dict:
        ...
        try:
            ...
            if step.action == "audio":
                ...
                # Duration-aware: size narration to the requested seconds so the
                # talking-avatar video length actually follows the duration param.
                req_dur = int((step.params or {}).get("duration") or 0)
                if is_talking and req_dur > 0:
                    from app.services.director_brain import fit_script_to_duration
                    script = await fit_script_to_duration(script, req_dur)
                script = script[:600]
                try:
                ...
                try:
                    ...
                    narrate = script
                    # Cap follows the requested duration (talking) instead of a flat 280.
                    max_chars = (max(280, int(req_dur * 4.5) + 16) if (is_talking and req_dur > 0) else (280 if is_talking else 180))
