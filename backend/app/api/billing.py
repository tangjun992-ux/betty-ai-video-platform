...
@router.post("/dev/grant", summary="[测试] 领取测试积分（仅测试/开发环境）")
async def dev_grant(amount: int = 50, db: AsyncSession = Depends(get_db),
                    user_id: int = Depends(resolve_user_id)):
    """Free credit top-up for the testing phase. Disabled in production when
    CREDITS_ENFORCED=true."""
    from app.config import settings
    is_dev = (os.getenv("ENV", "development").lower() != "production") or (not getattr(settings, "CREDITS_ENFORCED", True))
    if not is_dev:
        raise HTTPException(status_code=403, detail="测试领取仅在测试/开发环境开放")
    amount = max(1, min(int(amount or 50), 1000))
    bal = await _get_balance(db, user_id)
    bal.credits = int(bal.credits or 0) + amount
    db.add(bal)
    await db.commit()
    return {"ok": True, "granted": amount, "credits": int(bal.credits or 0)}
