...
class KieAdapter(BaseModelAdapter):
    ...
    async def generate_image(
        ...
        **kwargs,
        ...
        # Number of images. The unified gateway only reliably returns one
        # resultUrl per task, so we fan out one task per requested image and
        # return a list (matches the other adapters' count semantics).
        if count and count > 1:
            results_list = []
            for _ in range(min(int(count), 8)):
                r = await self._submit_and_poll(dict(payload), media_type="image", timeout=180)
                results_list.append(GenerationResult(
                    media_url=_extract_url(r, "imageUrl"),
                    media_type="image",
                    model=kie_id,
                    cost=_extract_kie_cost(r, "image"),
                    meta={"kie_task_id": r.get("taskId", ""), "resolution": size},
                ))
            return results_list
