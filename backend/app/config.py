...
class Settings(BaseSettings):
    ...
    CORS_ORIGINS: list[str] = [
        o.strip() for o in os.getenv(
            "CORS_ORIGINS",
            "http://localhost:3000,http://127.0.0.1:3000,http://localhost:3200,http://127.0.0.1:3200,http://localhost:3001,http://127.0.0.1:3001",
        ).split(",") if o.strip()
    ]
    ...
    # Billing / credits enforcement. Set CREDITS_ENFORCED=false to bypass the
    # credit gate during the pre-billing testing phase (deduct becomes a no-op).
    CREDITS_ENFORCED: bool = os.getenv("CREDITS_ENFORCED", "true").lower() in ("1", "true", "yes", "on")
