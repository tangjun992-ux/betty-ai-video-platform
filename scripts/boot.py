...
BACKEND = "/mnt/g/betty-ai-video/backend"
FE = "/home/tom/betty-frontend"

def log(m): print(m, flush=True)

# ---- 0. resolve venv python (Linux-fs venv to avoid WSL 9P corruption) ----
PY = "/home/tom/.venvs/betty-new/bin/python"
if not os.path.exists(PY):
    for alt in [f"{BACKEND}/.venv/bin/python",
                "/home/tom/ai-video-platform/backend/.venv/bin/python"]:
...
if not port_open(3000):
    spawn([f"{FE}/node_modules/.bin/next","dev","-p","3000"], FE, "/tmp/betty_fe.log",
          {"NEXT_PUBLIC_API_URL":"http://localhost:8000/api/v1","PORT":"3000"}); log("spawned next dev :3000")
else:
    log("frontend 3000 already up")
...
s2,_ = http("http://localhost:8000/api/openapi.json")
s3,_ = http("http://localhost:3000/", t=8)
try:
